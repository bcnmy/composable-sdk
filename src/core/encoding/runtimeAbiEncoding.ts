...
const encodeTuple = <const param extends AbiParameter & { components: readonly AbiParameter[] }>(
...
const encodeParams = (preparedParams: PreparedParam[]): (Hex | RuntimeValue)[] => {
  ...
  for (let i = 0; i < preparedParams.length; i++) {
    ...
    if (dynamic) {
      ...
      staticParams.push(numberToHex(staticSize + dynamicSize, { size: 32 }) as Hex);
...
export const getFunctionContextFromAbi = (functionSig: string, abi: Abi): FunctionContext => {
  if (abi.length === 0) {
